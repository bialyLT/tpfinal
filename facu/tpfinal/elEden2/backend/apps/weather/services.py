        ...
        for index, date_str in enumerate(dates):
            ...
            if index < len(weather_codes):
                try:
                    ...
                    weather_code = None
            results.append(DailyForecastSummary(
                date=date_obj,
...
class WeatherAlertService:
    ...
    def _ensure_localidad_coordinates(self, localidad) -> Optional[Tuple[float, float]]:
        ...
        try:
            ...
            logging.getLogger(__name__).warning('No se pudo persistir lat/lon para localidad %s', localidad.id_localidad)
        return lat, lon
    ...
    def _find_next_available_slot(self, fecha_inicial: datetime, empleados_necesarios: int) -> datetime:
        ...
        while dias_buscados < self.max_employee_search_days:
            ...
            empleados_ocupados = ReservaEmpleado.objects.filter(
                reserva_id__in=reservas_en_fecha
            ).values_list('empleado_id', flat=True)

            disponibles = Empleado.objects.filter(activo=True).exclude(
    ...
    def evaluate_reserva(self, reserva: Reserva, latitude: Optional[float] = None, longitude: Optional[float] = None, auto_create_alert: bool = True) -> dict:
        ...
        if localidad_info:
            localidad_payload = {
                'id': getattr(localidad_info['localidad'], 'id_localidad', None) if localidad_info.get('localidad') else None,
    ...
    def simulate_alert(self, reserva: Reserva, alert_date: Optional[datetime] = None, precipitation_mm: Optional[Decimal] = None, message: Optional[str] = None) -> WeatherAlert:
        ...
        if should_reassign:
            self._mark_reserva_requires_reprogramming(reserva, alerta, suggested_date, reason_message, decision['trigger'])
        return alerta
    ...
    def build_locality_forecasts(self, reservas, days: int = 7):
        ...
        for reserva in reservas:
            localidad_info = self._resolve_localidad_info(reserva)
            group = grouped.setdefault(localidad_info['key'], {
            ...
            group['reservas'].append({
                ...
                'cliente': f"{reserva.cliente.persona.nombre} {reserva.cliente.persona.apellido}" if getattr(reserva, 'cliente', None) and getattr(reserva.cliente, 'persona', None) else None,
        ...
        for group in grouped.values():
            ...
            forecast_payload = [
                {
                    'date': entry.date.date().isoformat() if isinstance(entry.date, datetime) else entry.date.isoformat(),
                    ...
                    'precipitation_sum_mm': float(entry.precipitation_sum) if entry.precipitation_sum is not None else None,
                }
...
class GeocodingService:
    ...
    def geocode_localidad(self, localidad) -> Optional[Tuple[float, float]]:
        ...
        try:
            ...
            logging.getLogger(__name__).warning('No se pudo geocodificar la localidad %s', getattr(localidad, 'id_localidad', 'desconocida'))
            return None
