...
from apps.users.models import Genero, TipoDocumento, Localidad, Persona, Cliente
...
class WeatherEndpointTests(TestCase):
	...
	def setUpTestData(cls):
		...
		cls.reprogramable_service = Servicio.objects.create(
			...
			reprogramable_por_clima=True
		)
		cls.non_reprogramable_service = Servicio.objects.create(
			nombre='Servicio fijo',
			reprogramable_por_clima=False
		)
		User = get_user_model()
	...
	def test_weather_check_creates_alert_for_reserva(self):
		...
		with patch(
			'apps.weather.services.WeatherClient.get_daily_forecast'
		) as mock_forecast, patch(
			'apps.emails.services.EmailService.send_weather_alert_notification'
		) as mock_send_email:
