...
class ConfiguracionPago(models.Model):
    ...
    def obtener_configuracion(cls):
        ...
        config, created = cls.objects.get_or_create(pk=1, defaults={"monto_sena": Decimal("50.00")})
        return config
...
class Reserva(models.Model):
    ...
    ESTADO_PAGO_CHOICES = [
        ...
        (
            "pendiente_pago_sena",
            "Pendiente de Pago de Seña",
        ),  # Reserva creada pero sin pago
...
class Diseno(models.Model):
    ...
    # Observaciones
    observaciones_cliente = models.TextField(blank=True, null=True, help_text="Comentarios del cliente sobre el diseño")
    notas_internas = models.TextField(blank=True, null=True, help_text="Notas internas del diseñador")
...
class DisenoProducto(models.Model):
    ...
    diseno = models.ForeignKey(Diseno, on_delete=models.CASCADE, related_name="productos")
    producto = models.ForeignKey("productos.Producto", on_delete=models.PROTECT, related_name="disenos")
    ...
    notas = models.TextField(blank=True, null=True, help_text="Notas sobre el uso del producto en el diseño")
...
class ImagenDiseno(models.Model):
    ...
    imagen = models.ImageField(upload_to="disenos/%Y/%m/", help_text="Imagen del diseño")
    descripcion = models.CharField(max_length=200, blank=True, null=True, help_text="Descripción de la imagen")
    orden = models.IntegerField(default=0, help_text="Orden de visualización de la imagen")
    fecha_subida = models.DateTimeField(auto_now_add=True)
...
class ImagenReserva(models.Model):
    ...
    reserva = models.ForeignKey(Reserva, on_delete=models.CASCADE, related_name="imagenes")
    imagen = models.ImageField(upload_to="reservas/%Y/%m/", help_text="Imagen del jardín o idea de referencia")
